  ...
  <?php 
    if(!is_single()){
      if(get_queried_object()){
        if(get_field('background', get_queried_object())){
          echo get_field('background', get_queried_object());
        } else {
      ...
      } elseif(get_field('background', wp_get_object_terms( get_the_id(), get_post_type().'_categories' )[0])) {
        echo get_field('background', wp_get_object_terms( get_the_id(), get_post_type().'_categories' )[0]);
      }
    }
  ?>
  )">
